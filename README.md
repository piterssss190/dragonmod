...
## Build toolchain
- Loom **1.17.20** (wersja przypięta na sztywno, nie "-SNAPSHOT" - pływająca
  wersja rozwiązywała się niedeterministycznie do różnych konkretnych buildów
  o różnym zachowaniu wobec bloku `mappings`; ta wersja nie remapuje już kodu
  - stąd `implementation` zamiast `modImplementation`, i zwykły `jar` zamiast
  `remapJar`)
- Gradle **9.4.0+**
